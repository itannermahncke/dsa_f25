 ...
 * Note: I implemented this myself, using the reference code as an interface and defining the functions on my own.
 * The only function I pulled from the reference code was BubbleDown().
 *
 ...
 */
class MinHeap<T> {
    // pair contains the data on the left-hand side and its weight on the right-hand side
    val vertices: MutableList<Pair<T, Double>> = mutableListOf()

    // index contains the data on the left-hand side and its index on the right-hand side
    val indexMap: MutableMap<T, Int> = mutableMapOf()
    ...
    fun getMin(): T? {
        when (vertices.size) {
            ...
            1 -> {
                val temp = vertices[0].first
                vertices.clear()
                return temp
            }
            // get the min value out and bubble down everything else
            else -> {
                val temp = vertices[0].first
    ...
    fun adjustHeapNumber(vertex: T, newNumber: Double) {
        getIndex(vertex)?.let {
            vertices[it] = Pair(vertices[it].first, newNumber)
            // both functions can be called as they end immediately if not needed
            percolateUp(startIndex=it)
            bubbleDown(startIndex=it)
        }
    }
     ...
     *
     * Given a starting index, recursively correct the place of the element at this index in the heap.
     * The element should not have a child with a smaller weight than itself.
     *
    ...
    private fun swap(index1: Int, index2: Int) {
        ...
        // now swap the items in the list
        val temp = vertices[index1]
     ...
     *
     * Given a starting index, recursively correct the place of the element at this index in the heap.
     * The element should not have a parent with a greater weight than itself.
     *
    ...
    private fun percolateUp(startIndex: Int) {
        ...
        val parentIndex = getParentIndex(startIndex)
        // if we reached the root
        if (parentIndex < 0) {
            return
        // if the element's parent has a greater weight than itself, recurse
        } else if (vertices[startIndex].second < vertices[parentIndex].second) {
            swap(parentIndex, startIndex)
            percolateUp(parentIndex)
        // if the elements are in the right place
        } else {
            return
        }
    }
